...
type (
	...
	// WARNING: The Postmark API returns the password in responses; treat this
	// struct as sensitive and avoid logging it.
...
// results are filtered to that stream via a properly URL-encoded query parameter.
...
func (a *API) ListWebhooks(messageStream string) (*ListWebhooksResp, error) {
	...
	if messageStream != "" {
		q := url.Values{}
		q.Set("MessageStream", messageStream)
		path += "?" + q.Encode()
	}
...
func (a *API) CreateWebhook(body *CreateWebhookReq) (*WebhookResp, error) {
	httpReq, err := a.newRequest(http.MethodPost, "webhooks", body)
	if err != nil {
		return nil, err
	}
	resp, err := a.Do(httpReq)
...
func (a *API) EditWebhook(webhookID int, body *EditWebhookReq) (*WebhookResp, error) {
	httpReq, err := a.newRequest(http.MethodPut, fmt.Sprintf("webhooks/%d", webhookID), body)
	if err != nil {
		return nil, err
	}
	resp, err := a.Do(httpReq)
